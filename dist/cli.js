...
program2.name("go-multi-auth").description("Manage OpenCode Go multi-account auth").version("0.1.0");
program2.command("list").description("Show all configured Go accounts").action(() => {
...
program2.command("add").description("Add a new Go account").option("-k, --key <key>", "Go API key").option("-l, --label <label>", "Account label").action((opts) => {
  if (!opts.key) {
...
program2.command("remove").description("Remove an account by number (1-based)").argument("<number>", "Account number to remove").action((numStr) => {
...
program2.command("status").description("Show rotation state and current account").action(() => {
  const data = loadAccounts();
